...
const TodoForm = ({addTodo}) =>{
    ...
    return(
        <div className={"container"}>
            <img src={require('./resources/logo.png')} width={500} height={100} className="text-center"></img>
            <div className="app-title text-center">Another ToDo App</div>
            <form onSubmit={handleSubmit} className={"form-group row todo-form"}>
                <input className={"form-control col-md-8 "} type={"text"} value={value} onChange={handleChange} tabIndex={-1}/>
                <input className={"form-control todo-button col-md-4 submit-button"} type={"submit"} value={"Add"}/>
            </form>
        </div>
    )
};
